...
const GerenciarTreinos = () => {
  ...
  const fazerRequisicao = async (endpoint, params = {}, method = 'GET', body = null) => {
    try {
  ...
  const buscarTreinos = async () => {
    ...
    try {
      ...
      const treinosProcessados = data.map(treino => ({
        ...treino,
        modalidadeNome: modalidadesRef[treino.ModalityId]?.nome || 'Modalidade Desconhecida',
  ...
  const handleSaveTreino = async (event) => {
    ...
    try {
      ...
      setTreinos(treinos.map(t => 
        t._id === currentTreino._id ? currentTreino : t
      ));
      
      setShowEditModal(false);
      alert('Treino atualizado no modo simulação! (Nota: as alterações não persistirão após recarregar a página, pois a API não suporta esta operação)');
  ...
  const handleSaveNewTreino = async (event) => {
    ...
    try {
      ...
      alert('Treino adicionado em modo simulação! (Nota: o treino não persistirá após recarregar a página, pois a API não suporta esta operação)');
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      alert('Treino excluído em modo simulação! (Nota: a exclusão não persistirá após recarregar a página, pois a API não suporta esta operação)');
              ...
              <table className="treinos-table">
                ...
                <tbody>
                  {treinosFiltrados.map((treino) => (
                    <tr key={treino._id}>
                      <td>
                        <span className="modalidade-tag">{treino.modalidadeTag}</span>
                        {treino.modalidadeNome}
                      </td>
                      <td>{formatarData(treino.StartTimestamp)}</td>
                      <td>{formatarData(treino.EndTimestamp)}</td>
                      <td>
                        <span className={`status-badge ${treino.Status.toLowerCase()}`}>
                          {treino.Status}
                        </span>
                      </td>
      ...
      {showDeleteModal && deletingTreino && (
        <div className="modal-overlay">
          <div className="delete-modal">
            ...
            <p>Tem certeza que deseja excluir o treino de <strong>{deletingTreino.modalidadeNome}</strong> agendado para <strong>{formatarData(deletingTreino.StartTimestamp)}</strong>?</p>
            <div className="modal-actions">
