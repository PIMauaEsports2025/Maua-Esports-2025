...
const GerenciarModalidades = () => {
  ...
  const obterImagemModalidade = (tag) => {
    ...
    const matchingTag = Object.keys(tagMap).find(
      (key) => tag && tag.includes(key)
    );
  ...
  const excluirModalidade = async (id) => {
    ...
    try {
      ...
      setTimeout(() => {
        setModalidades(
          modalidades.filter((modalidade) => modalidade.id !== id)
        );
        setDeletingId(null);
        ...
        {loading ? (
          ...
          <div className="modalidades-grid">
            {modalidades.length > 0 ? (
              ...
              <div className="no-results">Nenhuma modalidade encontrada</div>
            )}
          </div>
        )}
      </main>
